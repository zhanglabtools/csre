...
getBinnedScoreByChr <- function(chr, dirBw, nameMarks, nameSamples,
                                sizeBin, numBins, seqInfo,
                                chunk, level, dirNlp) {
  library(GenomicRanges)
  library(rtracklayer)
  library(rhdf5)
  ...
  writeTsrH5Metadata(chrFile, nameSamples, nameMarks)
...
tms <- Sys.time()
library(parallel)
numCores <- min(detectCores(), numChrs)
cl <- makeCluster(numCores)
clusterApply(cl, nameChrs, getBinnedScoreByChr,
             dirBw, nameMarks, nameSamples,
             sizeBin, numBins, seqInfo,
             chunk, level, dirNlp)
stopCluster(cl)
tme <- Sys.time()
